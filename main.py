...
from src.utils import DECK_A, DECK_B, DECK_C, DECK_D
...
def _parse_args(task_root: Path) -> TaskRunOptions:
    return parse_task_run_options(
        task_root=task_root,
        description="Run Iowa Gambling Task in human/qa/sim mode.",
...
def _summarize_trials(trials: list[dict], fallback_balance: int = 0) -> dict[str, float | int]:
    if not trials:
        return {
            "adv_rate": 0.0,
            "mean_rt_ms": 0.0,
            "timeout_count": 0,
            "net_sum": 0,
            "balance_end": int(fallback_balance),
            "deck_a_count": 0,
            "deck_b_count": 0,
            "deck_c_count": 0,
            "deck_d_count": 0,
    ...
    adv_count = sum(1 for row in responded if _as_bool(row.get("advantageous_choice", False)))
    adv_rate = (adv_count / len(responded)) if responded else 0.0
    ...
    net_sum = sum(_as_int(row.get("net_outcome", 0)) for row in trials)

    deck_a_count = sum(1 for row in responded if str(row.get("chosen_deck", "")).lower() == DECK_A)
    deck_b_count = sum(1 for row in responded if str(row.get("chosen_deck", "")).lower() == DECK_B)
    deck_c_count = sum(1 for row in responded if str(row.get("chosen_deck", "")).lower() == DECK_C)
    deck_d_count = sum(1 for row in responded if str(row.get("chosen_deck", "")).lower() == DECK_D)

    last_balance = int(fallback_balance)
    for row in reversed(trials):
        if row.get("balance_after", None) is not None:
            last_balance = _as_int(row.get("balance_after"), fallback_balance)
            break
    ...
    return {
        ...
        "net_sum": int(net_sum),
        "balance_end": int(last_balance),
        "deck_a_count": int(deck_a_count),
        "deck_b_count": int(deck_b_count),
        "deck_c_count": int(deck_c_count),
        "deck_d_count": int(deck_d_count),
    }
...
def _run_impl(*, mode: str, output_dir: Path | None, cfg: dict, participant_id: str):
    ...
    for block_i in range(total_blocks):
        ...
        block_summary = _summarize_trials(block.get_all_data(), fallback_balance=int(controller.total_money))
        if block_i < (total_blocks - 1):
            StimUnit("block", win, kb, runtime=trigger_runtime).add_stim(
                stim_bank.get_and_format(
                    ...
                    current_total=block_summary["balance_end"],
                    block_net=block_summary["net_sum"],
                    adv_rate=block_summary["adv_rate"],
                    mean_rt_ms=block_summary["mean_rt_ms"],
                    timeout_count=block_summary["timeout_count"],
                    deck_a_count=block_summary["deck_a_count"],
                    deck_b_count=block_summary["deck_b_count"],
                    deck_c_count=block_summary["deck_c_count"],
                    deck_d_count=block_summary["deck_d_count"],
                )
            ).wait_and_continue()

    overall = _summarize_trials(all_data, fallback_balance=int(controller.total_money))

    StimUnit("goodbye", win, kb, runtime=trigger_runtime).add_stim(
        stim_bank.get_and_format(
            "good_bye",
            total_trials=len(all_data),
            final_total=overall["balance_end"],
            net_total=overall["net_sum"],
            adv_rate=overall["adv_rate"],
            mean_rt_ms=overall["mean_rt_ms"],
            timeout_count=overall["timeout_count"],
            deck_a_count=overall["deck_a_count"],
            deck_b_count=overall["deck_b_count"],
            deck_c_count=overall["deck_c_count"],
            deck_d_count=overall["deck_d_count"],
        )
